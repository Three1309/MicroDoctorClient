...
public class UserShareHouseInfoActivity extends Activity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_usersharehouse);

        getIntentUserId = getIntent().getStringExtra("userId");
        whereStr = getIntent().getStringExtra("where");

        userDataStr = SharedPrefsUtil.getValue(UserShareHouseInfoActivity.this, APPConfig.USERDATA, "");
...
//        Toast.makeText(UserShareHouseInfoActivity.this, "dfddd", Toast.LENGTH_SHORT).show();
        ...
        listP.add(idParam);
        CustomWaitDialog.show(UserShareHouseInfoActivity.this,"获取数据。。。");
            ...
            public void run() {
                ...
                OkHttpUtils.post(APPConfig.findUserShareInfo, new OkHttpUtils.ResultCallback() {
                    ...
                    public void onFailure(Exception e) {
                        CustomWaitDialog.miss();
                        Toast.makeText(UserShareHouseInfoActivity.this, "服务器连接失败，请重试！", Toast.LENGTH_SHORT).show();
                    }
                },listP);
            }
        }).start();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.img_usersharehouse_back:
                if (whereStr.equals("ShareHouse")||whereStr.equals("ShareHouseListView")){
                    ShareHouseActivity.clickTop();
                }
                finish();
                break;
    ...
    private Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            shareHouseDto = gson.fromJson(result, ShareHouseDto.class);
            if (shareHouseDto != null) {
                if (shareHouseDto.getUserType() != 1) {
                    if (shareHouseDto.getUserNickName().equals("")) {
                        tv_name.setText("未填写");
                    }else
                        tv_name.setText(shareHouseDto.getUserNickName());
                    tv_type.setVisibility(View.INVISIBLE);
                }else {
                    if (shareHouseDto.getUserName().equals("")) {
                        tv_name.setText("未填写");
                    }
                    tv_name.setText(shareHouseDto.getUserName());
                }
                tv_myjianjie.setText("个人简介："+shareHouseDto.getUserJianjie());
                tv_collectAmount.setText(shareHouseDto.getCollectAmount()+"");
                tv_discussAmount.setText(shareHouseDto.getDiscussAmount()+"");
                tv_likesAmount.setText(shareHouseDto.getLikesAmount()+"");
                CustomWaitDialog.miss();
            }
