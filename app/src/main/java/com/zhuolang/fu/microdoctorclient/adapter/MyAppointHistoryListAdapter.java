...
public class MyAppointHistoryListAdapter extends BaseAdapter {
    ...
    //初始化把上下文，数据列表传递过来
    public MyAppointHistoryListAdapter(Context context, List<AppointmentDto> list) {
...
//        if (convertView == null) {
            convertView = inflater.inflate(R.layout.item_my_appointhistory, null);
            //第一次创建这个布局的话就寻找控件，记得是基于这个converView布局寻找
            holder.doctor_name = (TextView) convertView.findViewById(R.id.tv_item_my_appointhistory_doctor_name);
            holder.disease = (TextView) convertView.findViewById(R.id.tv_item_my_appointhistory_disease);
            holder.time = (TextView) convertView.findViewById(R.id.tv_item_my_apponinthistory_time);
            holder.diagnose = (TextView) convertView.findViewById(R.id.tv_item_my_appointhistory_diagnose);
            //第一次填充布局就缓存控件
...
//        Date date1 = TimeUtil.stringToDate(list.get(position).getDateTime());
        holder.time.setText("就诊时间："+TimeUtil.dateToStrNoTime(date));
        holder.diagnose.setText(list.get(position).getDiagnose());
