...
import com.zhuolang.fu.microdoctorclient.adapter.MySharesDiscussAdapter;
...
public class MySharesDiscussActivity extends Activity implements View.OnClickListener{
    ...
    private MySharesDiscussAdapter mySharesHistoryAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_mysharesdiscuss);

        getIntentUserId = getIntent().getStringExtra("userId");
//        whereStr = getIntent().getStringExtra("where");

        userDataStr = SharedPrefsUtil.getValue(MySharesDiscussActivity.this, APPConfig.USERDATA, "");
     ...
     */
    private void init(){
        img_back = (ImageView) findViewById(R.id.img_mysharesdiscuss_back);
        tv_top = (TextView) findViewById(R.id.tv_mysharesdiscuss_top);

        listView = (ListView) findViewById(R.id.mysharesdiscusslistview);

        if (!userId.equals(getIntentUserId)) {
            tv_top.setText("TA的评论");
        }
...
//        Toast.makeText(UserShareHouseInfoActivity.this, "dfddd", Toast.LENGTH_SHORT).show();
        ...
        listP.add(idParam);
        CustomWaitDialog.show(MySharesDiscussActivity.this,"获取数据。。。");
        new Thread(new Runnable() {
            @Override
            public void run() {
                //post方式连接  url
                OkHttpUtils.post(APPConfig.findMyDiscussShareInfo, new OkHttpUtils.ResultCallback() {
                    @Override
                    ...
                    public void onFailure(Exception e) {
                        ...
                        Toast.makeText(MySharesDiscussActivity.this, "服务器连接失败，请重试！", Toast.LENGTH_SHORT).show();
                    }
                },listP);
            }
        }).start();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.img_mysharesdiscuss_back:
                finish();
                break;
        }
    }
            ...
            switch (msg.what) {
                case 0:
                    if (result.equals("nodata")) {
                        Toast.makeText(MySharesDiscussActivity.this, "没有收藏的帖子", Toast.LENGTH_SHORT).show();
                    }else {
                        shareDtoList = gson.fromJson(result, new TypeToken<List<ShareDto>>() {}.getType());
                        if (shareDtoList != null && shareDtoList.size() > 0) {
                            mySharesHistoryAdapter = new MySharesDiscussAdapter(MySharesDiscussActivity.this,shareDtoList );
